...
- **MLflow Server**: http://192.168.1.86:5050
- **MinIO Web UI**: http://192.168.1.189:9001/browser/mlflow-artifacts
- **MinIO API**: http://192.168.1.189:9000 (not 9001!)

> **Note**: MinIO uses port 9000 for API access and port 9001 for the web UI.

## Quick Setup (5 minutes)

### Step 1: Get MinIO Credentials

1. Open MinIO web UI: http://192.168.1.189:9001
...
- Log experiments to your MLflow server (192.168.1.86:5050)
...
Open your MLflow UI:
```
http://192.168.1.86:5050
```

View artifacts in MinIO:
```
http://192.168.1.189:9001/browser/mlflow-artifacts
```
...
|----------|---------|---------|
| `MLFLOW_TRACKING_URI` | MLflow server URL | `http://192.168.1.86:5050` |
| `MLFLOW_S3_ENDPOINT_URL` | MinIO API endpoint | `http://192.168.1.189:9000` |
...
- Check MLflow server is running: `curl http://192.168.1.86:5050`
- Verify firewall allows access to port 5050

**Issue**: Cannot connect to MinIO
- Verify MinIO is running: `curl http://192.168.1.189:9000`
...
1. Open MinIO UI: http://192.168.1.189:9001
